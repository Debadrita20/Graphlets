...
void count4() {
	...
	for (int i=0;i<m;i++) {   //iterating over all edges
		...
		int x=edges[i].a, y=edges[i].b;  //taking endpoints of each edge: x is lower one as defined here
		//the nodes connected to a node are sorted in ascending order
		//so merge sort like technique for finding common nodes
	...
	frac_prev=-1;
	for (int x=0;x<n;x++) {   //iterating over each node
		frac = (100LL*x)/n;
		if (frac!=frac_prev) {
			printf("%d%%\r",frac);
			frac_prev=frac;
		...
		for (int nx=0;nx<deg[x];nx++) {
			...
			for (int ny=0;ny<deg[y];ny++) { //iterating over each adjacent node of y
				...
				neigh[nn++]=z;  // there exists an x-z edge
				//printf("Neighbour added: %d\n",z);
			}
			for (int i=0;i<nn;i++) { //iterating over each neighbour of x
				int z = neigh[i];
				...
				for (int j=i+1;j<nn;j++) {
					...
					if (adjacent(z,zz)) {
						C4[x]++; C4[y]++; C4[z]++; C4[zz]++;   //full graphlets
	...
	for (int x=0;x<n;x++) {
		...
		for (int nx1=0;nx1<deg[x];nx1++) {
			int y=inc[x][nx1].first, ey=inc[x][nx1].second; // y = adjacent node of x, ey = edge id
			for (int ny=0;ny<deg[y];ny++) {
				int z=inc[y][ny].first, ez=inc[y][ny].second; // z = adjacent node of y, ez = edge id
	...
	for(int i=0;i<m;i++)
	{
		...
		if(l==2)
		{
			for (int xi=0; xi<deg[x]; xi++) {
				int v=adj[x][xi];
				if(v==y) continue;
			if(!adjacent(v,y)&&(alreadydone.find(PAIR(x,v))==alreadydone.end())) 
			{ count++; alreadydone.insert(edges[i]);
			  //printf("dddffcount=%d --- %d %d %d\n",count,v,x,y);
			 }
		}
		for (int yi=0; yi<deg[y]; yi++) {
				int v=adj[y][yi];
				if(v==x) continue;
			if(!adjacent(v,x)&&(alreadydone.find(PAIR(y,v))==alreadydone.end())) 
			{ 
				count++; alreadydone.insert(edges[i]); 
				//printf("sssddcount=%d --- %d %d %d\n",count,v,x,y);
			}
		}
		}
		else 
				...
				for(int v1i=0;v1i<deg[v1];v1i++)
				{
					...
					{ 
					    ...
					    for(int ii=0;ii<deg[v2];ii++)
					   {
					    ...
					    for(int ii=0;ii<deg[y];ii++)
					    {
						int v3=adj[y][ii];
						if(v2==x||(alreadydone.find(PAIR(v3,y))!=alreadydone.end())) continue;
						if(!adjacent(v3,x)&&!adjacent(v3,v2)&&!adjacent(v3,v1))
						{
							count++; alreadydone.insert(edges[i]);
						}
					    }
				...
				for(int v1i=0;v1i<deg[v1];v1i++)
				{
					...
					{ 
					    ...
					    for(int ii=0;ii<deg[v2];ii++)
					   {
					    ...
					    for(int ii=0;ii<deg[x];ii++)
					    {
						int v3=adj[x][ii];
						if(v3==y||(alreadydone.find(PAIR(v3,x))!=alreadydone.end())) continue;
						if(!adjacent(v3,y)&&!adjacent(v3,v2)&&!adjacent(v3,v1))
						{
							count++; alreadydone.insert(edges[i]);
						}
					    }
...
int countFull(int x)
{
	...
	if(x==3)
	{
		for(int i=0;i<m;i++)
	    {
		    int x=edges[i].a, y=edges[i].b;
			for (int xi=0,yi=0; xi<deg[x] && yi<deg[y]; ) {
			if ((adj[x][xi]==adj[y][yi])&&(alreadydone.find(PAIR(x,adj[x][xi]))==alreadydone.end())&&(alreadydone.find(PAIR(y,adj[x][xi]))==alreadydone.end())) { count++; xi++; yi++; alreadydone.insert(edges[i]);}
			else if (adj[x][xi]<adj[y][yi]) { xi++; }
			else { yi++; }
		}
	    }
	...
	else if(x==4)
	{
		for(int i=0;i<m;i++)
	    {
			...
			for (int xi=0; xi<deg[x]; xi++) {
				...
				if(v2>=v1) break;
				if(v2==x) continue;
			if (adjacent(v1,v2)&&adjacent(v1,y)&&adjacent(v2,x)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())&&(alreadydone.find(PAIR(y,v1))==alreadydone.end())&&(alreadydone.find(PAIR(x,v2))==alreadydone.end())) 
			{ 
				count++; alreadydone.insert(edges[i]);
				//printf("gggg count= %d ffgfg %d %d %d %d",count,v1,v2,x,y);
			}
		    }
			}
	    }
	...
	else if(x==5)
	{
		for(int i=0;i<m;i++)
	    {
			...
			for (int xi=0; xi<deg[x]; xi++) {
				int v1=adj[x][xi];
				if(v1==y||!adjacent(v1,y)) continue;
				for(int yi=0; yi<deg[y];yi++){
				int v2=adj[y][yi];
				if(v2==v1||!adjacent(v2,x)) continue;
				if(v2==x) continue;
			if (adjacent(v1,v2)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())&&(alreadydone.find(PAIR(y,v1))==alreadydone.end())&&(alreadydone.find(PAIR(x,v2))==alreadydone.end())) 
			{ 
				for(int i1=0,j1=0;i1<deg[v1],j1<deg[v2];)
				{
					if(adj[v1][i1]==adj[v2][j1])
					{
						int vx=adj[v1][i1];
						if(adjacent(vx,x)&&adjacent(vx,y)&&(alreadydone.find(PAIR(vx,x))==alreadydone.end())&&(alreadydone.find(PAIR(vx,y))==alreadydone.end())&&(alreadydone.find(PAIR(vx,v2))==alreadydone.end())&&(alreadydone.find(PAIR(v1,vx))==alreadydone.end()))
						{
                            count++; alreadydone.insert(edges[i]);
						}
						i1++; j1++;
					}
					else if(adj[v1][i1]<adj[v2][j1])
					{
						i1++;
					}
					else j1++;
...
int countG4()
{
	...
	{
		int x=edges[i].a, y=edges[i].b;
		for(int xi1=0;xi1<deg[x];xi1++)
		{
			int v1=adj[x][xi1];
			if(v1==y||adjacent(v1,y)) continue;
			for(int xi2=0;xi2<deg[x];xi2++)
			{
				int v2=adj[x][xi2];
				if(v2>=v1) break;
				if(v2==y||adjacent(v2,y)||adjacent(v1,v2)) continue;
                if((alreadydone.find(PAIR(v1,x))==alreadydone.end())&&(alreadydone.find(PAIR(x,v2))==alreadydone.end()))
				{
					count++;
					alreadydone.insert(edges[i]);
				}
			}
		}
		for(int yi1=0;yi1<deg[y];yi1++)
		{
			int v1=adj[y][yi1];
			if(v1==x||adjacent(v1,x)) continue;
			for(int yi2=0;yi2<deg[y];yi2++)
			{
				int v2=adj[y][yi2];
				if(v2>=v1) break;
				if(v2==x||adjacent(v2,x)||adjacent(v1,v2)) continue;
                if((alreadydone.find(PAIR(v1,y))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end()))
				{
					count++;
					alreadydone.insert(edges[i]);
				}
	...
	for(int i=0;i<m;i++)
	{
		...
		for (int xi=0; xi<deg[x]; xi++) 
		{
			...
			for(int yi=0; yi<deg[y];yi++)
			{
				...
				if(v2==v1) continue;
				if(v2==x) continue;
				//printf("in 2nd loop%d %d %d %d\n",x,y,v1,v2);
			    if (adjacent(v1,v2)&&!adjacent(v1,y)&&!adjacent(v2,x)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())) 
			    { 
					//printf("heree\n");
				    count++; alreadydone.insert(edges[i]);
				    //printf("gggg count= %d ffgfg %d %d %d %d",count,v1,v2,x,y);
...
int countG6_12(int g)
{
	...
	for(int i=0;i<m;i++)
	{
		...
		for (int xi=0,yi=0; xi<deg[x] && yi<deg[y]; ) 
		{
			if ((adj[x][xi]==adj[y][yi])&&(alreadydone.find(PAIR(x,adj[x][xi]))==alreadydone.end())&&(alreadydone.find(PAIR(y,adj[x][xi]))==alreadydone.end())) 
			{ 
				int vv=adj[x][xi],flag1=0,flag2=0,flag3=0;
				for(int xi1=0;xi1<deg[x];xi1++)
				{
					int v=adj[x][xi1];
					if(v!=vv&&v!=y&&!adjacent(v,vv)&&!adjacent(v,y)&&(alreadydone.find(PAIR(x,v))==alreadydone.end()))
					{
                        flag1++;
						//printf("asddgg %d\n",count);
					}
				...
				{
					int v=adj[y][yi1];
					if(v!=vv&&v!=x&&!adjacent(v,vv)&&!adjacent(v,x)&&(alreadydone.find(PAIR(y,v))==alreadydone.end()))
					{
                        flag2++;
						//printf("asddgg %d\n",count);
					}
				}
				for(int i1=0;i1<deg[vv];i1++)
				{
					int v=adj[vv][i1];
					if(v!=x&&v!=y&&!adjacent(v,x)&&!adjacent(v,y)&&(alreadydone.find(PAIR(vv,v))==alreadydone.end()))
					{
                        flag3++;
						//printf("asddgg %d\n",count);
					}
				...
				if(g==12)
				{
				count=count+(flag1*flag2)+(flag2*flag3)*(flag1*flag3); 
				if(((flag1*flag2)+(flag2*flag3)*(flag1*flag3))>0)
				alreadydone.insert(edges[i]);
				}
				else
				{
					count=count+flag1+flag2+flag3;
	...
	for(int i=0;i<m;i++)
	{
		...
		for (int xi=0; xi<deg[x]; xi++) 
		{
			...
			for(int yi=0; yi<deg[y];yi++)
			{
				...
				if(v2==v1) continue;
				if(v2==x) continue;
			    if (adjacent(v1,v2)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())) 
			    { 
					if(adjacent(v1,y)&&!adjacent(v2,x)&&(alreadydone.find(PAIR(v1,y))==alreadydone.end()))
				    { count++; alreadydone.insert(edges[i]); }
                    else if(adjacent(v2,x)&&!adjacent(v1,y)&&(alreadydone.find(PAIR(v2,x))==alreadydone.end()))
					{ count++; alreadydone.insert(edges[i]); }
				    //printf("gggg count= %d ffgfg %d %d %d %d",count,v1,v2,x,y);
			    }
		    }
...
int countG10() //not done
{
	int count=0;
	set<PAIR> alreadydone;
	set<int> neighbours,neighboursx;
	for(int i=0;i<m;i++)
	{
		int x=edges[i].a, y=edges[i].b;
		for(int yi1=0;yi1<deg[y];yi1++)
		...
		{
			int v1=adj[x][xi1];
			if(v1==y||adjacent(v1,y)||(neighbours.find(v1)!=neighbours.end())) continue;
			for(int xi2=0;xi2<deg[x];xi2++)
			{
				int v2=adj[x][xi2];
				if(v2>=v1) break;
	...
	for(int i=0;i<m;i++)
	{
		...
		for (int xi=0; xi<deg[x]; xi++) 
		{
			...
			for(int yi=0; yi<deg[y];yi++)
			{
			    ...
			    if (adjacent(v1,v2)&&!adjacent(v1,y)&&!adjacent(v2,x)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())) 
			    { 
					for(int i1=0;i1<deg[x];i1++)
					{
						int v=adj[x][i1];
						if(v!=y&&v!=v1&&!adjacent(v,y)&&!adjacent(v,v1)&&!adjacent(v,v2))
						{
                            count++; alreadydone.insert(edges[i]);
						}
					}
	...
	for(int i=0;i<m;i++)
	{
		...
		for (int xi=0; xi<deg[x]; xi++) 
		{
			...
			for(int yi=0; yi<deg[y];yi++)
			{
			    ...
			    if (adjacent(v1,v2)&&(alreadydone.find(PAIR(v1,v2))==alreadydone.end())&&(alreadydone.find(PAIR(x,v1))==alreadydone.end())&&(alreadydone.find(PAIR(y,v2))==alreadydone.end())) 
			    { 
					int flagx=0,flagy=0,flagv1=0,flagv2=0;
					for(int j1=0;j1<deg[x];j1++)
					{
						int v=adj[x][j1];
						if(v!=y&&v!=v1&&v!=v2&&!adjacent(v,y)&&!adjacent(v,v1)&&!adjacent(v,v2))
						flagx++;
					}
					for(int j1=0;j1<deg[y];j1++)
					{
						int v=adj[y][j1];
						if(v!=x&&v!=v1&&v!=v2&&!adjacent(v,x)&&!adjacent(v,v1)&&!adjacent(v,v2))
						flagy++;
					}
					for(int j1=0;j1<deg[v1];j1++)
					{
						int v=adj[v1][j1];
						if(v!=y&&v!=x&&v!=v2&&!adjacent(v,y)&&!adjacent(v,x)&&!adjacent(v,v2))
						flagv1++;
					}
					for(int j1=0;j1<deg[v2];j1++)
					{
						int v=adj[v2][j1];
						if(v!=y&&v!=v1&&v!=x&&!adjacent(v,y)&&!adjacent(v,v1)&&!adjacent(v,x))
						flagv2++;
					}
					if(adjacent(v1,y)&&!adjacent(v2,x))
				    { 
                        if(g==17)
						{
							count+=(flagv1+flagy);
						...
						{
                            count+=(flagv2+flagx); alreadydone.insert(edges[i]);
						}						 
					}
                    else if(adjacent(v2,x)&&!adjacent(v1,y))
					{ 
						if(g==19)
						{
							count+=(flagv1+flagy);
...
int init(int argc, char *argv[]) {
	...
	if (argc!=5) {
		cerr << "Incorrect number of arguments. See below usage formats for details." << endl;
		cerr << "Usage 1: orca.exe [orbit type: node|edge] [graphlet size: 4/5] [graph - input file] [graphlets - output file]" << endl;
		cerr << "Usage 2: orca.exe graphlet [graphlet number, say 5 for G5] [graph - input file] [graphlets - output file]" << endl;
		return 0;
	}
	orbit_type = argv[1];
	if (orbit_type!="node" && orbit_type!="edge"&&orbit_type!="graphlet") {
		cerr << "Incorrect orbit type '" << argv[1] << "'. Should be 'node' or 'edge'." << endl;
		cerr << "Alternatively, if you want to count the number of particular graphlets, enter graphlet in this position" <<endl;
		return 0;
	}
	sscanf(argv[2],"%d",&GS);
	if (GS!=4 && GS!=5&&orbit_type!="graphlet") {
		cerr << "Incorrect graphlet size " << argv[2] << ". Should be 4 or 5." << endl;
		return 0;
	}
	else if(orbit_type=="graphlet"&&(GS<0||GS>29))
	{
		cerr << "Invalid Graphlet number. Only G0 till G29 available"<<endl;
		return 0;
	}
...
/*int main(int argc, char *argv[]) {
	...
	{
		printf("Counting number of graphlets - G%d \n\n",GS);
		int ans=0;
		if(GS==0||GS==1) ans=countLinear(GS+1);
		else if(GS==2) ans=countFull(3);
		else if(GS==3) ans=countLinear(3);
		else if(GS==4) ans=countG4();
		else if(GS==5) ans=countG5();
		else if(GS==6||GS==12) ans=countG6_12(GS);
		else if(GS==7) ans=countG7();
		else if(GS==8) ans=countFull(4);
		else if(GS==9) ans=countLinear(4);
		else if(GS==16) ans=countG16();
		else if(GS==17||GS==19) ans=countG17_19(GS);
		else if(GS==29) ans=countFull(5);
		fout<<ans<<endl;
		printf("Result: %d\n",ans);
	}
	return 0;
